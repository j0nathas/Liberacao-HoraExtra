...
public interface SolicitacaoFuncionariosRepository extends JpaRepository<SolicitacoesFuncionarios, Long> {
    ...
    @Query("SELECT sf FROM SolicitacoesFuncionarios sf WHERE sf.justificativa.solicitacoes.id = :solicitacaoId")
    List<SolicitacoesFuncionarios> findAllBySolicitacaoFilhaId(@Param("solicitacaoId") Long solicitacaoId);
